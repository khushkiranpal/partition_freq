 ...
 */
public class RMS_MOG_FREQ_U_SUM{
	...
	{
		System.out.println("STARTING MOG");
		...
		String filename= outputFolder+"primaryMOG"+"_"+inputfilename+"_"+date+".txt";
		String filename2= outputFolder+"energyMOG"+"_"+inputfilename+"_"+date+".txt";
	//	Writer writer_primary = new FileWriter(filename);
		...
		writer_energy.write("MOGTASKSET UTILIZATION FQ PRIMARY_ENERGY success fail"
				+ "\n");
		...
		while ((set = reader.nextTaskset()) != null)
		{
			...
			set_fq = SystemMetric.utilisation(taskset);//frequency.SysClockF(taskset);
...
*/
			...
			System.out.println("MOG    fq    "+fq +"    tasksets  "+total_no_tasksets+"  energy  "+ Double.valueOf(twoDecimals.format(primaryEnergy)));
	...
	//	writer_primary.close();
		...
		System.out.println("success MOG");
	}
