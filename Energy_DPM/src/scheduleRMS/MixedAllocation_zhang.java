 ...
 *  DYNAMIC 
 */
public class MixedAllocation_zhang {
		//GLOBAL PARAMETERS
...
*/	
			...
			boolean faultFromFile,double bcetRatio, long hyper, int n_proc) throws IOException
	{
		int m =n_proc;			///////////////// no. of processors/////////////////
		 System.out.println("STARTING MixedAllocation_zhang no. of processors "+m);
   ...
   String filename= outputFolder+"allocationMixed_zhang"+"_"+inputfilename+"_"+date;
    String filename4= outputFolder+"taskProcMixed_zhang"+"_"+inputfilename+"_"+date+".txt"; // TEMP USE
 	String filename1= outputFolder+"scheduleMixed_zhang"+"_"+inputfilename+"_"+date+".txt";
    String filename2= outputFolder+"energyMixed_zhang"+"_"+inputfilename+"_"+date+".txt";
    String filename3= outputFolder+"tasksMixed_zhang"+"_"+inputfilename+"_"+date+".txt";
    String filename5= outputFolder+"analysisMixed_zhang"+"_"+inputfilename+"_"+date+".txt";
    String filename7= outputFolder+"faultMixed_zhang"+"_"+inputfilename+"_"+date+".txt";
 ...
 //  Writer writer_taskProcWise = new FileWriter(filename4);
  ...
  //  int m =2;// no. of processors
    ...
    writer_energy.write("Mixed_zhangTASKSET UTILIZATION MIN_FREQ MAX_FREQ TOTAL_E ");
    	//	+ "P_ACTIVE_TIME P_IDLE_TIME P_SLEEP S_ACTIVE_TIME S_IDLE_TIME S_SLEEP");
      writer_tasks.write("Mixed_zhangfullBackupsExecuted partialBackupsExecuted fullBackupsCancelled"
  ...
  //  writer_taskProcWise.write("proc primary  backup  total"); //  TEMP USE
		writer_fault.write("\nMixed_zhangTASKSET TIME PRI_PROC QUE TASKID JOBID WCET PROM DEADLINE");
  ...
  //  	 long hyper =SystemMetric.hyperPeriod(taskset);  /////////////// HYPER PERIOD////////////
    	...
    	SysClockFreq frequencySYS = new SysClockFreq();
   ...
   //	partition.allocation_Priority(taskset, freeProcList, filename);
 	...
 	/////////////////FREQUENCY ALLOCATION//////////////////
		...
		//	//System.out.println("   load   " +load);
			double slack = Math.max(0, (LLB_N- load));//Math.max(0, (1- load));
  	...
  	for(Processor p : freeProcList)
		{
		double load = (SystemMetric.utilisation(p.taskset));
		double set_fq = frequencySYS.SysClockF(taskset);
		double	LLB_N = p.taskset.size()*(Math.pow(2, ((double)1/(double)(p.taskset.size())))-1);
			...
			fq=load/LLB_N;
			  for(ITask tp : p.taskset)
              {
              	//if(tp.isPrimary())
              	tp.setFrequency(fq);
                  	System.out.println("p "+p.getId()+" t "+tp.getId()+" f "+fq);
              }
			  
		 ps.set_freq(p.taskset,Double.valueOf(twoDecimals.format(fq)));   // set frequency
		 ps.setResponseTime(p.taskset);
 		ps.setPromotionTime(p.taskset);
		}
     ...
     /*	//TEMP FAULT INDUCTION
    	...
    	tempFault.add((long)46);    	tempFault.add((long)47); 
...
//		//System.out.println("activationTimes  "+activationTimes.size()+"  promotionTimes  "+promotionTimes.size());
		...
		while(itr.hasNext())
	  	...
	  	*/
		 writer_schedule.write("\nMixed_zhangP_ID TASKID FREQ WCET WCET_o ACET BCET DEADLINE P/B RESP promo\n");
		...
		for(Processor p : freeProcList)
		{
			for(ITask t :p.taskset)
			{
				if(t.getResponseTime()==0)
					  response_zero++;
				writer_schedule.write("\n"+p.getId()+" "+t.getId()+" "+t.getFrequency()+" "+t.getWcet()+" "
						+t.getWCET_orginal()+" "+t.getACET() 
	...
	//	writer_analysis.write("P_ID TASKID JOBID PR/BK TIME");
        ...
        //			//System.out.println(" time  "+time);
        		...
        		//		//System.out.println("time  "+time+"   backup job in ready queue  "+newBJob.getTaskId()+"  job  "+newBJob.getJobId());
        			...
        			while(itr1.hasNext())
        			{
        				...
        				System.out.println("time ready queue  "+time +"  proc  "+proc.getId()+"  task "+jR.getTaskId()+
        						...
        						+"  promo  time   "+jR.getPromotionTime()+" exec freq "+jR.getExec_frequency());
        			}
        	...
        	for (Processor proc : freeProcList)
        	{
        		...
        		if(time>0 && !proc.readyQueue.isEmpty()  &&
        				...
        				proc.getCurrentJob().getPeriod()>proc.readyQueue.first().getPeriod()&& proc.isBusy())
        		{
     ...
     //   	System.out.println("TIME  "+time+"  p  "+proc.getId()+"  idle "+ proc.idleTime);;
      ...
      /*  for(Processor p : freeProcList)
        ...
        System.out.println("  mixed_zhang fq    "+maxfq +"  tasksets  "+total_no_tasksets+" energy  "+energyTotal);
 ...
 // writer_allocation.close();
  ...
  //  writer_taskProcWise.close();
    System.out.println("finish mixed_zhang");
	}
