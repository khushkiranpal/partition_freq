 ...
 */
public class RMS_ZHANGmin_freq{
	...
	{
		System.out.println("STARTING ZHANGmin_freq");
		...
		String filename= outputFolder+"primaryZHANGmin_freq"+"_"+inputfilename+"_"+date+".txt";
		String filename2= outputFolder+"energyZHANGmin_freq"+"_"+inputfilename+"_"+date+".txt";
	//	Writer writer_primary = new FileWriter(filename);
		...
		writer_energy.write("ZHANGmin_freqTASKSET UTILIZATION fq PRIMARY_ENERGY SUCCESS fail\n");
...
*/
			...
			if(sys_freq_temp<CRITICAL_freq)
				...
				primary.setFrequency(Double.valueOf(twoDecimals.format( discreteFreq(sys_freq_temp))));
				...
				//	boolean waking = false;//10-12-18
					...
					//	System.out.println(" time "+time+" activeJobQ "+activeJobQ.size()+" t "+j_temp.getTaskId()+" j "+j_temp.getJobId());
						if(!freq_set_tasks.contains(j_temp.getTaskId()) )
						{
							...
							{
							//	System.out.println("time  "+ time+"  primary.getFrequency() "+primary.getFrequency());
								primary.setFrequency(Double.valueOf(twoDecimals.format(j_temp.getFrequency()
										+primary.getFrequency())));
...
*/
	...
	/*	//14-12-18
			...
			if(deadlineMissed)
				...
				writer_energy.write(total_no_tasksets++ + " "+Double.valueOf(twoDecimals.format(U_SUM))+" "
						+Double.valueOf(twoDecimals.format(fq	))+" "+Double.valueOf(twoDecimals.format(primary.getEnergy_consumed()))+" 1"+" 0"+"\n");
			System.out.println("ZHANGmin_freq   fq    "+"    tasksets  "+total_no_tasksets+"  energy  "+ Double.valueOf(twoDecimals.format(primary.getEnergy_consumed())));
	...
	//	writer_primary.close();
		...
		System.out.println("success ZHANGmin_freq");
	}
