...
public class RMS_Sysclock{
	...
	{
		System.out.println("STARTING rms");
		...
		String filename= outputFolder+"primaryRMS"+"_"+inputfilename+"_"+date+".txt";
		String filename2= outputFolder+"energyRMS"+"_"+inputfilename+"_"+date+".txt";
	//Writer writer_primary = new FileWriter(filename);
		...
		writer_energy.write("RMSTASKSET UTILIZATION FQ PRIMARY_ENERGY success fail"
				+ "\n");
		...
		//	double LLB_N = taskset.size()*(Math.pow(2, ((double)1/(double)(taskset.size())))-1);
			...
			set_fq = frequency.SysClockF(taskset);
...
*//*
			...
			System.out.println("rms    fq    "+fq +"    tasksets  "+total_no_tasksets+"  energy  "+ Double.valueOf(twoDecimals.format(primaryEnergy)));
	...
	//	writer_primary.close();
		...
		System.out.println("success rms");
	}
