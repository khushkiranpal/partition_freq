 ...
 *  DYNAMIC 
 */
public class MixedDPM_primaryfreqProm {
	//GLOBAL PARAMETERS
			...
			boolean faultFromFile,double bcetRatio, long hyper, int n_proc) throws IOException
	{
		...
		System.out.println("STARTING MixedAllocation_DPM primaryfreqProm  no. of processors "+m);
		...
		String filename= outputFolder+"allocationMixed_DPMprimaryfreqProm  "+"_"+inputfilename+"_"+date;
		String filename4= outputFolder+"taskProcMixed_DPMprimaryfreqProm  "+"_"+inputfilename+"_"+date+".txt"; // TEMP USE
		String filename1= outputFolder+"scheduleMixed_DPMprimaryfreqProm  "+"_"+inputfilename+"_"+date+".txt";
		String filename2= outputFolder+"energyMixed_DPMprimaryfreqProm  "+"_"+inputfilename+"_"+date+".txt";
		String filename3= outputFolder+"tasksMixed_DPMprimaryfreqProm  "+"_"+inputfilename+"_"+date+".txt";
		String filename5= outputFolder+"analysisMixed_DPMprimaryfreqProm  "+"_"+inputfilename+"_"+date+".txt";
		String filename7= outputFolder+"faultMixed_DPMprimaryfreqProm  "+"_"+inputfilename+"_"+date+".txt";
		...
		writer_energy.write("MixedDPMprimaryfreqProm TASKSET UTILIZATION MIN_FREQ MAX_FREQ TOTAL_E ");
		//	+ "P_ACTIVE_TIME P_IDLE_TIME P_SLEEP S_ACTIVE_TIME S_IDLE_TIME S_SLEEP");
		writer_tasks.write("MixedDPMprimaryfreqProm fullBackupsExecuted partialBackupsExecuted fullBackupsCancelled"
		...
		writer_fault.write("\nMixedprimaryfreqDPMProm TASKSET TIME PRI_PROC QUE TASKID JOBID WCET PROM DEADLINE");
		...
		while ((set = reader.nextTaskset()) != null) // SCHEDULING STARTS FOR ALL TASKSETS IN FILE
		{
			...
			double fq = 1, minfq=1, maxfq=0.1;	double fq_backup;
			...
			//partition.allocPrimBack_Prioritywise(taskset, freeProcList, filename);
			//	partition.allocation_Prioritywise(taskset, freeProcList, filename);
			partition.allocation_M_WFD(taskset, freeProcList, filename);
			//	partition.allocation_Back_Prio(taskset, freeProcList, filename);
			//	partition.allocation_Priority(taskset, freeProcList, filename);
		...
		/*	for(Processor p : freeProcList)
			{
				...
				for (ITask t: p.taskset)
				{
					...
					fq = ((double)t.getWCET_orginal()/(double)t.getPeriod());
					if(t.isPrimary())
						t.setFrequency(fq/newUtiPrimary);
					else
						t.setFrequency(1);
					System.out.println(" t "+t.getId()+" fq "+fq+" f "+t.getFrequency());
				}
				/// setting DPM frequency////////////end/
			...
			*/
			...
			for(Processor p : freeProcList)
			{
				...
				/////setting promotion time for     fq=u/llb         /////////
				double load = (SystemMetric.utilisation(p.taskset));
				fq_backup = load/LLB_N;
				ps.set_freq_MixedAlloc(p.taskset,Double.valueOf
						(twoDecimals.format(fq_backup)));
				ps.setResponseTimeForMWFD(p.taskset);
				ps.setPromotionTime(p.taskset); 
			...
			for (ITask t: p.taskset)
			{
				...
				if(t.isPrimary())
					...
					t.setFrequency(fq_backup);//ui/utiBackup
...
*/
		...
		while(itr.hasNext())
			...
			for (Processor proc : freeProcList)
			{
				...
				if (!proc.backupJobQueue.isEmpty())
				{
					...
					System.out.println("time  "+time+"   backup job in ready queue  "+newBJob.getTaskId()+"  job  "+newBJob.getJobId()+" f "+newBJob.getFrequency());
					newBJob.setExec_frequency(newBJob.getFrequency());
					promoted_task = true;
				...
				while (itr1.hasNext())
				{
					...
					j_temp.setExec_frequency(j_temp.getFrequency());
			...
			//		if(proc.getId()==2 && j_temp.getTaskId()==324)
				...
				{//////////////setting frequency//////////////
					...
					{
						Job jR = itr12.next();
						jR.setExec_frequency(jR.getFrequency());
						/*	System.out.println("time ready queue  "+time +"  proc  "+proc.getId()+"  task "+jR.getTaskId()+
							"   job  "+jR.getJobId() +"  p/B  "+jR.isPrimary()+"  arrivaal  "+jR.getActivationDate()
							+"  promo  time   "+jR.getPromotionTime()+" f "+jR.getFrequency()+
					...
					//		System.out.println("n while t "+j_temp.getTaskId()+" f "+j_temp.getExec_frequency());
						...
						if (proc.getCurrentJob()!=null && proc.isBusy()) {
							...
							if (remain_time>0) {
								...
								proc.getCurrentJob().setExec_frequency(proc.getCurrentJob().getFrequency());
					...
					{//////////////setting frequency//////////////
						...
						if (proc.getCurrentJob()!=null && proc.isBusy()) {
							...
							if (remain_time>0) {
								...
								if(proc.getCurrentJob().isPrimary())
								proc.getCurrentJob().setExec_frequency(SystemMetric.utilisation(proc.taskset));
								else
								proc.getCurrentJob().setExec_frequency(SystemMetric.utilisation(proc.taskset));
          ...
          //  	System.out.println("out fault time  "+time+"  TEMPFAULT SIZE "+tempFault.size()+"  tempProcCheck size  "+tempProcCheck.size());
				...
				for (Processor proc : freeProcList)
				{
					if(proc.isBusy())
					{
						...
						{
							System.out.println("getCurrentJob deadline missed  task id "+proc.getCurrentJob().getTaskId()+" job id " + proc.getCurrentJob().getJobId()
			...
			/*  for(Processor p : freeProcList)
        {
			...
			System.out.println("  mixed_DPM primaryfreqProm  fq    "+maxfq +"  tasksets  "+total_no_tasksets+" energy  "+energyTotal);
		...
		System.out.println("finish mixed_DPMprimaryfreq Prom ");
	}
