...
public class RMS_LLB_N{
	...
	{
		System.out.println("STARTING RMS_LLB_N");
		...
		String filename= outputFolder+"primaryRMS_LLB_N"+"_"+inputfilename+"_"+date+".txt";
		String filename2= outputFolder+"energyRMS_LLB_N"+"_"+inputfilename+"_"+date+".txt";
	//	Writer writer_primary = new FileWriter(filename);
		...
		writer_energy.write("RMS_LLB_NTASKSET UTILIZATION FQ PRIMARY_ENERGY success fail"
				+ "\n");
		...
		while ((set = reader.nextTaskset()) != null)
		{
			...
			double set_fq = 0, fq = 0;
			double LLB_N = taskset.size()*(Math.pow(2, ((double)1/(double)(taskset.size())))-1);
		
			set_fq = SystemMetric.utilisation(taskset)/LLB_N;//frequency.SysClockF(taskset);
			//System.out.println("set_fq  "+set_fq  +" frequency.SysClockF(taskset)  "+frequency.SysClockF(taskset));
			set_fq= discreteFreq(set_fq);
...
*/
			...
			primaryEnergy = energyConsumed.energyActive(primary.activeTime, fq)+energy_consumed;
					//energyConsumed.energy_IDLE(primary.idleTime) +
					//energyConsumed.energySLEEP(primary.sleepTime) +energy_consumed;
			...
			System.out.println("RMS_LLB_N    fq    "+fq +"    tasksets  "+total_no_tasksets+"  energy  "+ Double.valueOf(twoDecimals.format(primaryEnergy)));
	...
	//	writer_primary.close();
		...
		System.out.println("success RMS_LLB_N");
	}
