 ...
 *  DYNAMIC 
 */
public class MixedAllocation_DPM {
	//GLOBAL PARAMETERS
			...
			boolean faultFromFile,double bcetRatio, long hyper, int n_proc) throws IOException
	{
		...
		System.out.println("STARTING MixedAllocation_DPM  no. of processors "+m);
		...
		String filename= outputFolder+"allocationMixed_DPM "+"_"+inputfilename+"_"+date;
		String filename4= outputFolder+"taskProcMixed_DPM "+"_"+inputfilename+"_"+date+".txt"; // TEMP USE
		String filename1= outputFolder+"scheduleMixed_DPM "+"_"+inputfilename+"_"+date+".txt";
		String filename2= outputFolder+"energyMixed_DPM "+"_"+inputfilename+"_"+date+".txt";
		String filename3= outputFolder+"tasksMixed_DPM "+"_"+inputfilename+"_"+date+".txt";
		String filename5= outputFolder+"analysisMixed_DPM "+"_"+inputfilename+"_"+date+".txt";
		String filename7= outputFolder+"faultMixed_DPM "+"_"+inputfilename+"_"+date+".txt";
		...
		writer_energy.write("MixedTASKSET UTILIZATION MIN_FREQ MAX_FREQ TOTAL_E ");
...
*/
			...
			for(Processor p : freeProcList)
			{
				...
				ps.set_freq_MixedAlloc(p.taskset,1);
				ps.setResponseTimeForMWFD(p.taskset);
				ps.setPromotionTime(p.taskset); 
			...
			for (ITask t: p.taskset)
			{
				...
				fq = ((double)t.getWCET_orginal()/(double)t.getPeriod());
				if(t.isPrimary())
					t.setFrequency(fq/(1-utiBackup));// (LLB_N-utiBackup)//LLB_N
				else
					t.setFrequency(fq/utiBackup);
				System.out.println(" t "+t.getId()+" fq "+fq+" f "+t.getFrequency());
			}
			}
...
*/
			...
			/*  for(Processor p : freeProcList)
        {
			...
			System.out.println("  mixed_DPM  fq    "+maxfq +"  tasksets  "+total_no_tasksets+" energy  "+energyTotal);
		...
		System.out.println("finish mixed_DPM ");
	}
