 ...
 *  DYNAMIC 
 */
public class MixedAllocationMoghaddaas {
		//GLOBAL PARAMETERS
...
*/	
			...
			boolean faultFromFile,double bcetRatio, long hyper, int n_proc) throws IOException
	{
		int m =n_proc;			///////////////// no. of processors/////////////////
		 System.out.println("STARTING MixedAllocation no. of processors "+m);
   ...
   String filename= outputFolder+"allocationMixed"+"_"+inputfilename+"_"+date;
    ...
    String filename7= outputFolder+"faultMixed"+"_"+inputfilename+"_"+date+".txt";
  ...
  //  	 long hyper =SystemMetric.hyperPeriod(taskset);  /////////////// HYPER PERIOD////////////
    ...
    /*    	Reliability reliab = new Reliability();
        ///////setRelib_Mogdhaddas(ArrayList<ITask> taskset, double fMin, double freq, int d)
     	...
     	double PoF_f ;
     	fq=1;
     	while(fq>=CRITICAL_freq)
     	{
     	reliab.setRelib_Mogdhaddas(taskset, CRITICAL_freq, fq, d);
     	PoF_f = 1- reliab.reliabilitySystem(taskset);
    	...
    	+ " \n PoF_f  "+String.format("%.16f", PoF_f));
     	...
     	reliable_freq=fq;
     	System.out.println("reliable_freq  "+reliable_freq);
     	*/
 	...
 	//partition.allocPrimBack_Prioritywise(taskset, freeProcList, filename);
    	...
    	partition.allocatio_WFD(taskset, freeProcList, filename,1);
   ...
   //	partition.allocation_Priority(taskset, freeProcList, filename);
	//	break;
 	...
 	/////////////////FREQUENCY ALLOCATION//////////////////
  	...
  	fq=Math.max(CRITICAL_freq, SystemMetric.utilisation(taskset));
  	if(fq>0.9 && fq<=1)
  		fq=1;
  	if(fq>0.8 && fq<=0.9)
  		fq=0.9;
  	...
  	ps.setResponseTime_Moghdass(p.taskset);
	...
	//	writer_analysis.write("P_ID TASKID JOBID PR/BK TIME");
     ...
     //   	System.out.println("TIME  "+time+"  p  "+proc.getId()+"  idle "+ proc.idleTime);;
      ...
      /*  for(Processor p : freeProcList)
        ...
        System.out.println("  mixed fq    "+maxfq +"  tasksets  "+total_no_tasksets+" energy  "+energyTotal);
