 ...
 *  DYNAMIC 
 */
public class MixedAllocation_BACKUP_DELAYING_over {
	//GLOBAL PARAMETERS
			...
			boolean faultFromFile,double bcetRatio, long hyper, int n_proc) throws IOException
	{
		...
		System.out.println("STARTING MixedAllocation_BACKUP_DELAYING_OVERno. of processors "+m);
		...
		String filename= outputFolder+"allocationMixed_BACKUP_D_OVER"+"_"+inputfilename+"_"+date+".txt";
		String filename4= outputFolder+"taskProcMixed_BACKUP_D_OVER"+"_"+inputfilename+"_"+date+".txt"; // TEMP USE
		String filename1= outputFolder+"scheduleMixed_BACKUP_D_OVER"+"_"+inputfilename+"_"+date+".txt";
		String filename2= outputFolder+"energyMixed_BACKUP_D_OVER"+"_"+inputfilename+"_"+date+".txt";
		String filename3= outputFolder+"tasksMixed_BACKUP_D_OVER"+"_"+inputfilename+"_"+date+".txt";
		String filename5= outputFolder+"analysisMixed_BACKUP_D_OVER"+"_"+inputfilename+"_"+date+".txt";
		 String filename7= outputFolder+"faultMixed_BACKUP_D_OVER"+"_"+inputfilename+"_"+date+".txt";
	...
	//	int m =2;// no. of processors
		...
		Instance ints = new Instance();
		  writer_energy.write("Mixed_BACKUP_D_OVERTASKSET UTILIZATION MIN_FREQ MAX_FREQ TOTAL_E"
		  		+ " P_ACTIVE_TIME P_IDLE_TIME P_SLEEP S_ACTIVE_TIME S_IDLE_TIME S_SLEEP");
			writer_tasks.write("Mixed_BACKUP_D_OVERfullBackupsExecuted partialBackupsExecuted fullBackupsCancelled"
		...
		//  writer_taskProcWise.write("proc primary  backup  total"); //  TEMP USE
			writer_fault.write("\nMixed_backup_OVERTASKSET TIME PRI_PROC QUE TASKID JOBID WCET PROM DEADLINE");
	...
	/*		
	    	...
	    	ArrayList<Long> tempProcCheck = new ArrayList<Long>();
	    		tempFault.add((long)78880);     
	    		tempFault.add((long)87504); 
	tempFault.add((long)108515);	        	tempFault.add((long)124125);  
...
*/
				...
				/*		
						if( time>1085289 && time< 1085300)//!proc.getCurrentJob().isPrimary() && 
								System.out.println("new polling time  "+time+ "  p  "+proc.getId()+" task "+proc.getCurrentJob().getTaskId() +
        					"   job  "+proc.getCurrentJob().getJobId()+ 
        					" isCompletionSuccess() "+proc.getCurrentJob().isCompletionSuccess()+
        					 "  p/b "+proc.getCurrentJob().isPrimary()+"  prom " +proc.getCurrentJob().getPromotionTime()
        					 +"\n dead "+proc.getCurrentJob().getDeadline()+ " activation "+proc.getCurrentJob().getActivationDate()+" wcet "+proc.getCurrentJob().getRomainingTimeCost()+" preempt "+proc.getCurrentJob().isPreempted);
				*/		 
			        						...
			        						while(spareItr.hasNext())
			        						{
			        							...
			        							if(temp1.getTaskId()== p.getCurrentJob().getTaskId() && temp1.getJobId()== p.getCurrentJob().getJobId())
			        							{
			            							...
			            							//	System.exit(0);
			            								...
			            								p.getCurrentJob().getBackupProcessor().backupJobQueue.addJob(temp1);
			            									System.out.println(" REVISED PROMOTION TIME AT "+ time+"  backupJobQueue ");
			                    							System.out.println("time  "+time+" after   "+temp1.getPromotionTime()+"  jobid  "+temp1.getJobId()+"  period  "+temp1.getPeriod()+" d "+temp1.getAbsoluteDeadline());			
			        					...
			        					//			System.out.println("noOfFaults "+noOfFaults+" is primary "+temp1.isPrimary());
			        						...
			        						while(spareItready.hasNext())
			        						{
			        							...
			        							if(temp1.getTaskId()== p.getCurrentJob().getTaskId() && temp1.getJobId()== p.getCurrentJob().getJobId())
			        							{
			        								...
			        								System.out.println("temp1.getJobId() "+temp1.getJobId()+"  temp1.getPeriod() "+temp1.getPeriod()+
			        										" "+(temp1.getJobId())*temp1.getPeriod());
			        									System.out.println("REVISED PROMOTION TIME AT "+ time+" readyQueue ");
			                							System.out.println("time  "+time+" after   "+temp1.getPromotionTime()+"  jobid  "+temp1.getJobId()+"  period  "+temp1.getPeriod()+" d "+temp1.getAbsoluteDeadline());
...
*/
				...
				for (Processor proc : freeProcList)
				{
		        	if(proc.isBusy())
		        	{
						...
						{
							 System.out.println("current job   deadline missed p "+proc.getId()+" p/b "+proc.getCurrentJob().isPrimary());
							System.out.println("deadline missed  task id "+proc.getCurrentJob().getTaskId()+" job id " + proc.getCurrentJob().getJobId()
									 ...
									 +"  time "+time + "  prom "+ proc.getCurrentJob().getPromotionTime());
		        		...
		        		while(itrtemp.hasNext())
		        			...
		        			//	System.out.println("t  "+jtemp.getTaskId()+"   job  "+jtemp.getJobId());
		        				...
		        				if (jtemp!=null && !jtemp.isCompletionSuccess() && jtemp.getAbsoluteDeadline()<time) // IF TIME IS MORE THAN THE DEADLINE, ITS A MISSING DEADLINE
		        				{
       							 ...
       							 +"\tactivation "+jtemp.getActivationDate()+"  deadline time  "+jtemp.getAbsoluteDeadline()+" wcet "+jtemp.getRomainingTimeCost()
       							 +"  time "+time);
		        					System.out.println(" backup queue time   "+time+"  p   "+proc.getId()+"   proc.readyQueue size   "+proc.readyQueue.size()+ " backupJobQueue size "+proc.backupJobQueue.size());
							...
							//				 System.out.println("hp i  "+in.getHighPriorTask()+" lp k "+in.getLowPriorTask()+"  n i k "+in.getNoOfInstances());
								...
								//						 System.out.println("nota hp i  "+ink.getHighPriorTask()+" lp k "+ink.getLowPriorTask()+" [ n i k ]"+ink.getNoOfInstances());
										...
										//				  System.out.println("setting  bk   "+bk.getTaskId()+"   prom  time"+bk.getPromotionTime()+"  primary  "+bk.isPrimary());
													...
													/*	 if (bk.getTaskId()==282 && bk.getJobId()==1086)
														 System.out.println("1857 tempProc "+tempProc.getId()+"   backupJobQueue size "+tempProc.backupJobQueue.size()+"   readyQueue  "+ tempProc.readyQueue.size());
*/
											...
											//				 System.out.println("1439  remove "+rem);
													 ...
													 tempProc.backupJobQueue.addJob(bk);
												/*	 if (bk.getTaskId()==282 && bk.getJobId()==1086)
														 System.out.println("1905 tempProc "+tempProc.getId()+"   backupJobQueue size "+tempProc.backupJobQueue.size()+"   readyQueue  "+ tempProc.readyQueue.size());
*/
										//			 System.out.println("setting  bk   "+bk.getTaskId()+"   prom  time"+bk.getPromotionTime());
													  ...
													  if(nkl<=nki && bk.upperQ)
														 {
															 ...
															 else
															 {
																 ...
																 //			 System.out.println("tempProc "+tempProc.getId()+"   backupJobQueue size "+tempProc.backupJobQueue.size()+"   readyQueue  "+ tempProc.readyQueue.size());
																/* if (bk.getTaskId()==282 && bk.getJobId()==1086)
																	 System.out.println("2075 tempProc "+tempProc.getId()+"   backupJobQueue size "+tempProc.backupJobQueue.size()+"   readyQueue  "+ tempProc.readyQueue.size());
*/
																 tempProc.backupJobQueue.addJob(bk);
																 tempProc.readyQueue.remove(bk);
															 }
														 ...
														 else if(nkl<=nki && !bk.upperQ)	
														 {
															 ...
															 tempProc.backupJobQueue.addJob(bk);
															 if (bk.getTaskId()==282 && bk.getJobId()==1086)
																 System.out.println("2119 tempProc "+tempProc.getId()+"   backupJobQueue size "+tempProc.backupJobQueue.size()+"   readyQueue  "+ tempProc.readyQueue.size());
											  ...
											  if(nkl<=nki && bk.upperQ)
												 {
													 ...
													 else
													 {
														 ...
														 //			 System.out.println("tempProc "+tempProc.getId()+"   backupJobQueue size "+tempProc.backupJobQueue.size()+"   readyQueue  "+ tempProc.readyQueue.size());
													/*	 if (bk.getTaskId()==282 && bk.getJobId()==1086)
															 System.out.println("2292 tempProc "+tempProc.getId()+"   backupJobQueue size "+tempProc.backupJobQueue.size()+"   readyQueue  "+ tempProc.readyQueue.size());
*/
														 tempProc.backupJobQueue.addJob(bk);
														 tempProc.readyQueue.remove(bk);
													 }
												 ...
												 else if(nkl<=nki && !bk.upperQ)	
												 {
													...
													//		 System.out.println("1807  remove "+rem);
													 ...
													 tempProc.backupJobQueue.addJob(bk);
													/* if (bk.getTaskId()==282 && bk.getJobId()==1086)
														 System.out.println("2336 tempProc "+tempProc.getId()+"   backupJobQueue size "+tempProc.backupJobQueue.size()+"   readyQueue  "+ tempProc.readyQueue.size());
*/
													 //			  //System.out.println("setting  bk   "+bk.getTaskId()+"   prom  time"+bk.getPromotionTime());
													 //
												 }
...
*/
        ...
        writer_taskProcWise.write("\n "+p.getId()+" "+p.getNoOfPriJobs()+" "+p.getNoOfBackJobs()+
			...
			System.out.println("MixedAllocation_BACKUP_DELAYING_over fq    "+maxfq +"  tasksets  "+total_no_tasksets+" energy  "+energyTotal);
	...
	//	writer_schedule.close();
		...
		System.out.println("finish MixedAllocation_BACKUP_DELAYING_OVER");
	}
