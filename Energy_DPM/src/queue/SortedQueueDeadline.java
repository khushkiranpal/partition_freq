 ...
 * @author KIRAN
 */
public class SortedQueueDeadline implements ISortedQueue{
    private final TreeSet<ITask> tasks;
	private int id;

	protected Comparator<ITask> comparator;

	protected SortedQueueDeadline(Comparator<ITask> compar) {
		this.tasks = new TreeSet<ITask>(compar);
		comparator = compar;
	}
	
	public SortedQueueDeadline(){
		this(new Comparator<ITask>() {
			public int compare(ITask t1, ITask t2) {
				long comp  = t1.getDeadline() - t2.getDeadline();
    ...
    public ISortedQueue cloneTaskSet() {
		ISortedQueue set = new SortedQueueDeadline(comparator);
    ...
    public ISortedQueue newInstance() {
		return new SortedQueueDeadline(comparator);
	}
